...
ABOUT_PROJECT_URL = (
    ...
    "1Dq0MJL1hrdNG4XYo-BNKMrmRfnO3-ndeTo42vg1GjUM/edit?usp=sharing"
)
ABOUT_PROJECT_TEXT = (
    "О проекте:\n\n"
    "Информация о проекте и правилах работы доступны по ссылке:\n"
    f"{ABOUT_PROJECT_URL}"
)


START_NOTICE_TEXT = (
    "Здравствуйте, это бот по регистрации магистерских проектов на кафедре КОЗМ МУА!\n"
    "Уважаемые пользователи! При обращении к боту и регистрационной гугл таблице требуется время. "
    "Обычно после нажатия на кнопку Старт до ответа бота может проходить до 1 минуты. "
    "Наберитесь терпения. Также задержки ответов могут быть при вызове других команд. "
    "Это Сервис Google временно ограничивает число запросов к таблице (лимит на минуту)."
)
...
def _student_help_text(*, require_phone_pin: bool) -> str:
    ...
    return (
        ...
        "/help — эта справка.\n\n"
        f"{pin_note}"
        "При повторном обращении к боту в анкете поле можно пропустить, если вы его "
        f"ранее заполнили праильно: отправьте {SKIP_TOKEN} или /skip.\n"
        "Если нужно прервать текущий диалог, отправьте /cancel."
    )
...
HELP_REPLY_TEXT_SUPERVISOR = (
    ...
    "/status ФИО — проверка проекта подопечного по листу «Магистранты»\n"
    "/unreg — список подопечных, которые ещё не зарегистрировались\n"
    "/reg_list — список зарегистрированных подопечных\n"
    "/about — информация о проекте\n\n"
    "Служебные команды: /help — эта справка; /cancel — прервать текущий диалог.\n\n"
    "Проверка отчётов и анкета самого магистранта — в сценарии магистранта (/start)."
)
...
def default_bot_commands() -> list[BotCommand]:
    ...
    return [
        ...
        BotCommand("supervisor_message", "Сообщение научруку"),
