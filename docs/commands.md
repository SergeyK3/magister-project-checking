...
* Пользовательские инструкции и меню используют латиницу; русские slash-команды ниже оставлены только как compatibility routes.
...
| ------- | ----- | ---------- |
| `/start` | `/старт` | регистрация и вход в сценарий |
| `/spravka` | `/справка` | проверка / справка (зависит от роли) |
| `/cancel` | `/выход` | отмена внутри `ConversationHandler` |
...
| Команда | Назначение |
| ------- | ---------- |
| `/spravka` | проверка **своей** строки в «Регистрация», если аккаунт там есть (как у магистранта) |
...
Указать в `/spravka номер_строки` или `/spravka Чужое ФИО` могут **только администраторы** — см. `spravka_start`.
...
| `/supervisor_message` | стандартное сообщение научруку по строкам `Магистранты`, где `Регистрация = нет` |
